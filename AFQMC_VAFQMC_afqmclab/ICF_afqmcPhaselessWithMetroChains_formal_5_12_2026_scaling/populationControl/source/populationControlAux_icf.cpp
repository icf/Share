...
#include "../include/populationControl_icf.h"

using namespace std;

//Determine the number each thread need to send out to other thread
//send_number_list is the list only in main thread.
void deterSendNumber_icf(int& send_number, vector<int>& send_number_list, const vector<int>& table)
{
...
void deterTableSend_icf(int send_number, const vector<int>& send_number_list, const vector<int>& table, vector<int>& table_send)
{
    int rank=MPIRank(); int size=MPISize();
...
int deterBufSendNumber_icf(int L_chunk, int send_number, const vector<int>& table_send)
{
    int count=0;
...
int deterBufRecvNumber_icf(const vector<int>& table_recv)
{
