...
#include "../../include/Metropolis2s/JastrowProjector2s.h"
#include "afqmclab.h"

using namespace std;
using namespace tensor_hao;

JastrowProjector2s::JastrowProjector2s() {
}

JastrowProjector2s::JastrowProjector2s(JastrowProjector2s &x) { copy_deep(x); }
JastrowProjector2s &JastrowProjector2s::operator=(JastrowProjector2s &x) { return *this; }

JastrowProjector2s::~JastrowProjector2s() { }


void JastrowProjector2s::initialParameters(Metropolis2sMethod method)
{
    ...
    for(int jastrow=1-1; jastrow<=numOfJastrow-1; jastrow++){
        if(getVariableName(jastrow)=="generalHamiltonian_HAFQMC_icf"){
            ...
            //////////////////////////////////////////////////////////////////////////////////////
            expMinusDtK_Jastrow_vec[jastrow]= model_Jastrow[jastrow].returnExpMinusAlphaK2s_nonHermitian(  1, "fixedOrder", JastrowExpM[jastrow], 10e-8, 0  );
            //////////////////////////////////////////////////////////////////////////////////////
        }
...
void JastrowProjector2s::initialParametersTwoJastrow(Metropolis2sMethod method)
{
    //////////////////////////////////////////////////////////////
    ...
    for(int jastrow=1-1; jastrow<=numOfJastrow/2-1; jastrow++){
        if(getVariableName(jastrow)=="generalHamiltonian_HAFQMC_icf"){
            ...
            const string stringTemp2="constForce_param_"+to_string(jastrow);
            constForce_Jastrow[jastrow] = expMinusDtV_Jastrow_vec[jastrow].readForce(stringTemp2);
            expMinusDtK_Jastrow_vec[jastrow]= model_Jastrow[jastrow].returnExpMinusAlphaK2s_nonHermitian(  1.0, "fixedOrder", JastrowExpM[jastrow], 10e-8, 0      );
            //
        }
        else if(getVariableName(jastrow)=="generalHamiltonian_K_HAFQMC_icf"){
            const string stringTemp="model_Jastrow_param_"+to_string(jastrow);
...
string JastrowProjector2s::getVariableName( int i_temp)
{
    return variableName_vec[i_temp];
}

int JastrowProjector2s::getVariableSite( int i_temp)
{
    int i=i_temp;
    return i;
}

void JastrowProjector2s::extendMetroChainToRight( JastrowProjector2s jastrowProjector_input)
{
...
void JastrowProjector2s::copy_deep(JastrowProjector2s &x)
{
