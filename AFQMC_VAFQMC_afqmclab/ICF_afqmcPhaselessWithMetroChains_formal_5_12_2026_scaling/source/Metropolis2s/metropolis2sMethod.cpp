...
//
#include "../../include/Metropolis2s/metropolis2sMethod.h"

using namespace std;

Metropolis2sMethod::Metropolis2sMethod()
{
    setDefault();
}

Metropolis2sMethod::Metropolis2sMethod(const Metropolis2sMethod &x) { copy_deep(x); }

Metropolis2sMethod::Metropolis2sMethod(Metropolis2sMethod &&x) { move_deep(x); }
...
Metropolis2sMethod &Metropolis2sMethod::operator=(const Metropolis2sMethod &x) { copy_deep(x); return *this; }

Metropolis2sMethod &Metropolis2sMethod::operator=(Metropolis2sMethod &&x) { move_deep(x); return *this; }

void Metropolis2sMethod::read(const string &filename)
{
...
void Metropolis2sMethod::print()
{
    ...
    cout<<setw(36)<<"BPMetroStabilizeStep "<<setw(26)<<BPMetroStabilizeStep<<endl;
...
#ifdef MPI_HAO
void MPIBcast(Metropolis2sMethod &buffer, int root, MPI_Comm const &comm)
{
...
void Metropolis2sMethod::pack(vector<char> &buf, int &posit) const
{
    ...
    MPI_Pack(BPMetroUpdateType.c_str(), BPMetroUpdateType.size(), MPI_CHAR, buf.data(), buf.size(), &posit, MPI_COMM_WORLD);
...
void Metropolis2sMethod::unpack(const vector<char> &buf, int &posit)
{
...
void Metropolis2sMethod::setDefault()
{
    BPMetroSampleCap = 5;
...
void Metropolis2sMethod::copy_deep(const Metropolis2sMethod &x)
{
...
void Metropolis2sMethod::move_deep(Metropolis2sMethod &x)
{
    JastrowSlice = x.JastrowSlice;
    JastrowName = x.JastrowName;
    JastrowExpM = x.JastrowExpM;
